...
class TileImage:
    def __init__(self,
                 img_scale=(1024,1024), #image size
                 crop_size=(512,512), #tile size
                 offset_h=[0,512,0,512],
                 offset_w=[0,512,512,0],
                 idx=None,
                 scale_factor=None,):
    ...
    def _crop_data(self, results, offset_h, offset_w):
        crop_size = self.crop_size
        i = self.idx
        if i is None: 
            return results
        for key in results.get('img_fields', ['img']):
            img = results[key] #h:[0,512,0,512],w:[0,512,512,0]
            crop_y1, crop_y2 = offset_h[i], offset_h[i] + crop_size[0]
            crop_x1, crop_x2 = offset_w[i], offset_w[i] + crop_size[1]
        ...
        for key in results.get('bbox_fields', []):
            # e.g. gt_bboxes and gt_bboxes_ignore
            bbox_offset = np.array([offset_w[i], offset_h[i], offset_w[i], offset_h[i]],
                                dtype=np.float32)
            bboxes = results[key] - bbox_offset
            if self.bbox_clip_border:
                bboxes[:, 0::2] = np.clip(bboxes[:, 0::2], 0, img_shape[1])
    ...
    def __call__(self, results):
        ...
        """
        image_size = results['img'].shape[:2]
        results = self._crop_data(results, self.offset_h, self.offset_w)
        return results
